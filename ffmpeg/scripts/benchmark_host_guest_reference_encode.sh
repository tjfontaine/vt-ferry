...
VM_NAME="${VM_NAME:-vt-ferry-ref-bench-$$}"
WORKDIR="${WORKDIR:-${REPO_ROOT}/artifacts/ffmpeg-reference-benchmark-$$}"
# 1080p reference encodes keep multiple frames in flight; 4 slots is too
# few and trips kCVReturnError (-6660) "Could not create pixel buffer from
# pool" within a few frames. 32 matches the auto-bump in
# prove_smolvm_videotoolbox_encode.sh for the same reason.
...
# ffmpeg's `-benchmark` emits a line like:
#   bench: utime=2.152s stime=0.110s rtime=0.346s
# extract_bench_field pulls out one field across the most recent line.
# We use it to recover utime (user CPU) and stime (system CPU) for the
# CPU-efficiency comparison — VideoToolbox's main value over libx264 is
# burning ~0.7 CPU cores instead of ~6, not raw wallclock fps.
extract_bench_field() {
  python3 - "$1" "$2" <<'PY'
import pathlib
import re
...
HOST_SW_MP4="${WORKDIR}/host-libx264.mp4"
HOST_SW_LOG="${WORKDIR}/host-libx264.log"
HOST_VT_MP4="${WORKDIR}/host-videotoolbox.mp4"
HOST_VT_LOG="${WORKDIR}/host-videotoolbox.log"
...
if [ "${USE_VT_FERRY_IMAGE}" = "1" ]; then
  GUEST_FFMPEG_BIN="/opt/vt-ferry/bin/ffmpeg"
  GUEST_SHIM_LIBDIR=""
else
  GUEST_FFMPEG_BIN="$(guest_repo_path "${GUEST_FFMPEG_BIN}")" || die "GUEST_FFMPEG_BIN must be under ${REPO_ROOT}"
...
"${HOST_FFMPEG_BIN}" -hide_banner -benchmark -y \
  ...
  -c:v libx264 \
  ${SOFTWARE_ARGS} \
  "${HOST_SW_MP4}" >"${HOST_SW_LOG}" 2>&1

echo "host: h264_videotoolbox"
"${HOST_FFMPEG_BIN}" -hide_banner -benchmark -y \
  -i "${REFERENCE_VIDEO}" \
  -map 0:v:0 \
  -an \
...
# /usr/bin/ffmpeg is the distro libavcodec baseline; in image
# mode /opt/vt-ferry/bin/ffmpeg is also on PATH and would shadow
# bare 'ffmpeg'.
smolvm_cmd machine exec --name "${VM_NAME}" -- sh -lc \
  "/usr/bin/ffmpeg -hide_banner -encoders | grep -q 'libx264'"
...
smolvm_cmd machine exec --name "${VM_NAME}" -- sh -lc \
  "${GUEST_LD_PREFIX} ${GUEST_FFMPEG_BIN} -hide_banner -encoders | grep -q 'h264_videotoolbox'"

echo "guest: libx264"
smolvm_cmd machine exec --name "${VM_NAME}" -- sh -lc \
  "/usr/bin/ffmpeg -hide_banner -benchmark -y -i ${GUEST_REFERENCE_VIDEO} -map 0:v:0 -an ${frame_limit_args} -vf format=nv12 -c:v libx264 ${SOFTWARE_ARGS} ${GUEST_WORKDIR}/$(basename "${GUEST_SW_MP4}") > ${GUEST_WORKDIR}/$(basename "${GUEST_SW_LOG}") 2>&1"

echo "guest: h264_videotoolbox (vsock)"
smolvm_cmd machine exec --name "${VM_NAME}" -- sh -lc \
  "env ${GUEST_VT_TRANSPORT_ENV} ${GUEST_LD_PREFIX} ${GUEST_FFMPEG_BIN} -hide_banner -benchmark -y -i ${GUEST_REFERENCE_VIDEO} -map 0:v:0 -an ${frame_limit_args} -vf format=nv12 -c:v h264_videotoolbox ${VIDEOTOOLBOX_ARGS} ${GUEST_WORKDIR}/$(basename "${GUEST_VT_MP4}") > ${GUEST_WORKDIR}/$(basename "${GUEST_VT_LOG}") 2>&1"

for path in \
  "${HOST_SW_MP4}" "${HOST_VT_MP4}" "${GUEST_SW_MP4}" "${GUEST_VT_MP4}" \
  "${HOST_SW_LOG}" "${HOST_VT_LOG}" "${GUEST_SW_LOG}" "${GUEST_VT_LOG}"
do
  require_file "${path}"
done
...
# user + system CPU time per encoder. utime/rtime is the canonical
# "how many cores were busy on average" metric for the run.
...
# Use the *encoded* frame count for fps and realtime ratios. With
# `-frames:v N` ffmpeg encodes exactly N frames; without a limit it
# encodes the whole reference. Computing fps against the reference
# total when only `N` frames were actually encoded inflates fps by
# (reference_total / N) — that's how earlier runs reported 55k fps
# for libx264 at 1080p.
encoded_frames = frame_limit if frame_limit > 0 else reference_frame_count
# Source is constant 30 fps for the bundled reference. Recover the
# encoded duration from frame count + source fps so x_realtime is
# the meaningful "wallclock spent vs realtime spent" ratio.
source_fps = (reference_frame_count / reference_duration_s) if reference_duration_s > 0 else 0.0
encoded_duration_s = (encoded_frames / source_fps) if source_fps > 0 else reference_duration_s
...
def cores(cpu_s, rtime_s):
    """Average cores busy across the run. utime+stime is total CPU
    seconds spent across all threads/cores; dividing by wallclock
    gives the equivalent fully-busy-core count."""
    return (cpu_s / rtime_s) if rtime_s > 0 else 0.0

def cpu_efficiency(vt_cpu, sw_cpu):
    """Fraction of libx264's CPU cost that VideoToolbox spends.
    A value of 0.10 means VT does the same encode for 10% of the
    CPU. Lower is better (libx264 is the baseline at 1.00)."""
...
summary = {
    ...
    "host": {
        "libx264_rtime_s": host_sw,
        "videotoolbox_rtime_s": host_vt,
        "libx264_cpu_s": host_sw_cpu,
        "videotoolbox_cpu_s": host_vt_cpu,
        "libx264_avg_cores": cores(host_sw_cpu, host_sw),
        "videotoolbox_avg_cores": cores(host_vt_cpu, host_vt),
        "videotoolbox_speedup_vs_libx264": host_sw / host_vt,
        "libx264_x_realtime": encoded_duration_s / host_sw,
        "videotoolbox_x_realtime": encoded_duration_s / host_vt,
    },
    "guest": {
        "transport": "vsock",
        "libx264_rtime_s": guest_sw,
        "videotoolbox_rtime_s": guest_vt,
        "libx264_cpu_s": guest_sw_cpu,
        "videotoolbox_cpu_s": guest_vt_cpu,
        "libx264_avg_cores": cores(guest_sw_cpu, guest_sw),
        "videotoolbox_avg_cores": cores(guest_vt_cpu, guest_vt),
        "videotoolbox_speedup_vs_libx264": guest_sw / guest_vt,
        "libx264_x_realtime": encoded_duration_s / guest_sw,
        "videotoolbox_x_realtime": encoded_duration_s / guest_vt,
    },
    "overhead": {
        "guest_vs_host_libx264_ratio": guest_sw / host_sw,
        "guest_vs_host_videotoolbox_ratio": guest_vt / host_vt,
        "guest_videotoolbox_extra_seconds_vs_host": guest_vt - host_vt,
    },
    "value_of_hardware_encode": {
        # Hardware (VideoToolbox) saves CPU even when it doesn't beat
        # libx264 on raw fps. Both ratios are "VT cost / libx264 cost"
        # — lower is better, and a ratio < 1.0 says VT is cheaper for
        # the same output.
        "host_vt_cpu_efficiency_vs_libx264": cpu_efficiency(host_vt_cpu, host_sw_cpu),
        "guest_vt_cpu_efficiency_vs_libx264": cpu_efficiency(guest_vt_cpu, guest_sw_cpu),
    },
}

if encoded_frames > 0:
    summary["host"]["libx264_fps"] = encoded_frames / host_sw
    summary["host"]["videotoolbox_fps"] = encoded_frames / host_vt
    summary["guest"]["libx264_fps"] = encoded_frames / guest_sw
    summary["guest"]["videotoolbox_fps"] = encoded_frames / guest_vt
...
print("")
print(f"Encoded {encoded_frames} frames ({encoded_duration_s:.3f}s of source).")
print("")
print("Host:")
print(f"  libx264_rtime_s: {host_sw:.3f}")
print(f"  h264_videotoolbox_rtime_s: {host_vt:.3f}")
print(f"  videotoolbox_speedup_vs_libx264: {host_sw / host_vt:.3f}x")
print(f"  libx264_x_realtime: {encoded_duration_s / host_sw:.3f}x")
print(f"  videotoolbox_x_realtime: {encoded_duration_s / host_vt:.3f}x")
print(f"  libx264_cpu_s: {host_sw_cpu:.3f} ({cores(host_sw_cpu, host_sw):.2f} cores avg)")
print(f"  videotoolbox_cpu_s: {host_vt_cpu:.3f} ({cores(host_vt_cpu, host_vt):.2f} cores avg)")
if encoded_frames > 0:
    print(f"  libx264_fps: {encoded_frames / host_sw:.3f}")
    print(f"  videotoolbox_fps: {encoded_frames / host_vt:.3f}")
print("")
print("Guest:")
print(f"  libx264_rtime_s: {guest_sw:.3f}")
print(f"  h264_videotoolbox_rtime_s: {guest_vt:.3f}")
print(f"  videotoolbox_speedup_vs_libx264: {guest_sw / guest_vt:.3f}x")
print(f"  libx264_x_realtime: {encoded_duration_s / guest_sw:.3f}x")
print(f"  videotoolbox_x_realtime: {encoded_duration_s / guest_vt:.3f}x")
print(f"  libx264_cpu_s: {guest_sw_cpu:.3f} ({cores(guest_sw_cpu, guest_sw):.2f} cores avg)")
print(f"  videotoolbox_cpu_s: {guest_vt_cpu:.3f} ({cores(guest_vt_cpu, guest_vt):.2f} cores avg)")
if encoded_frames > 0:
    print(f"  libx264_fps: {encoded_frames / guest_sw:.3f}")
    print(f"  videotoolbox_fps: {encoded_frames / guest_vt:.3f}")
print("")
print("Value of Hardware Encode (lower CPU cost ratio = better):")
host_eff = cpu_efficiency(host_vt_cpu, host_sw_cpu)
guest_eff = cpu_efficiency(guest_vt_cpu, guest_sw_cpu)
if host_eff is not None:
    print(f"  host_vt_cpu_vs_libx264: {host_eff:.3f} "
          f"({(1.0 - host_eff) * 100:.1f}% CPU saved)")
if guest_eff is not None:
    print(f"  guest_vt_cpu_vs_libx264: {guest_eff:.3f} "
          f"({(1.0 - guest_eff) * 100:.1f}% CPU saved)")
print("")
print("Guest Overhead vs Host:")
print(f"  libx264_ratio: {guest_sw / host_sw:.3f}x")
print(f"  videotoolbox_ratio: {guest_vt / host_vt:.3f}x")
print(f"  guest_videotoolbox_extra_seconds: {guest_vt - host_vt:.3f}")
print("")
print("Notes:")
print("  guest libx264 baseline uses the distro ffmpeg inside the VM")
print("  guest h264_videotoolbox uses the shimmed ffmpeg build over vsock")
print("")
